...
exports.withdrawlAmount = function(accountnumber, amount, callback){
    var newBalance = 0;
    console.log(accountnumber);
    User.findOne({account: accountnumber},function(error, user){
        if(error){
...
exports.userBalance = function(account, password, callback){
    User.findOne({account: account, password: password}, function(error, user){
        ...
        console.log(user);
        callback(null, { balance: user.balance, account: user.account , name: user.custName});
    });
};

exports.getUserAccounts = function(email, callback){
